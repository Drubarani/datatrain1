...
def main():
    ...
    print(f"Datasets Loaded Successfully.Rows:{df.shape[0]},Features:{df.shape[1]}\n")

    # Clean up any duplicate header rows and convert columns to numeric
    df = df[df['H'] != 'H'].copy()
    ...
    df["LogRuns"] = np.log1p(df["R"].fillna(0))
